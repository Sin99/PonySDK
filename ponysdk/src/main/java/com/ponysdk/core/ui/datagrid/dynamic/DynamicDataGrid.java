...
public class DynamicDataGrid<DataType> extends DataGrid<DataType> {
    ...
    public DynamicDataGrid(final Configuration configuration, final View view, final Function<DataType, ?> keyProvider, final Comparator<DataType> comparator) {
    ...
    private void initColumnDescriptor() {
        Predicate<Method> filter = configuration.getFilter();
    ...
    private void addColumnDescriptor(final Method method) {
        ...
        Function<String, String> captionTransform = configuration.getCaptionTransform();
        final ColumnDescriptor<DataType> columnDescriptor = ColumnDescriptor.newDefault(captionTransform.apply(method.getName()), data -> this.invoke(method, data));
